...
Require-SourcePattern 'src\RenderThread.cpp' 'RemoveFromQueue\(nPage\)' 'per-page job de-duplication'
...
Require-SourcePattern 'src\RenderThread.cpp' 'const bool queueCleanupAfterCurrent = job\.type == CLEANUP' 'cleanup queueing behind running foreground work'
Require-SourcePattern 'src\RenderThread.cpp' 'The same render became useful again before completion' 'same-identity running-render revival'
Require-SourcePattern 'src\RenderThread.cpp' 'm_bRejectCurrentJob = false' 're-entry clears only the exact render rejection'
Require-SourcePattern 'src\RenderThread.cpp' 'return left\.request == right\.request' 'single RenderRequest identity comparison'
...
Require-SourcePattern 'src\RenderThread.cpp' 'cleanupPages contains requests made by this update, not a whitelist' 'persistent cleanup lifecycle rule'
Require-SourcePattern 'src\RenderThread.cpp' 'it->type == CLEANUP \|\|' 'cleanup retention outside the current request set'
Require-SourcePattern 'src\RenderThread.cpp' 'windows\.readInfoPages\.find\(it->GetPage\(\)\)' 'read-info cleanup cancellation on re-entry'
