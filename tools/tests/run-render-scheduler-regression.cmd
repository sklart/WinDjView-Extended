...
set "TEST_OUTPUT_DIR=tools\tests"
set "TEST_BASENAME=%TEST_OUTPUT_DIR%\render_scheduler_regression"
cl /nologo /W4 /EHsc /MT /DNDEBUG /DWIN32 /DWIN64 /D_WINDOWS /D_CONSOLE /D_UNICODE /DUNICODE /DHAS_WCTYPE=1 /DTHREADMODEL=WINTHREADS /DDO_CHANGELOCALE=0 /DWIN32_MONITOR /DNEED_JPEG_DECODER /DLIBDJVU_STATIC /D_CRT_SECURE_NO_DEPRECATE /D_CRT_NONSTDC_NO_DEPRECATE /D_SECURE_SCL=0 /I"src" /I"src\libdjvu" /Fo"%TEST_BASENAME%.obj" /c "tools\tests\render_scheduler_regression.cpp"
if errorlevel 1 exit /b %errorlevel%
cl /nologo /W4 /EHsc /MT /DNDEBUG /DWIN32 /DWIN64 /D_WINDOWS /D_CONSOLE /D_UNICODE /DUNICODE /DHAS_WCTYPE=1 /DTHREADMODEL=WINTHREADS /DDO_CHANGELOCALE=0 /DWIN32_MONITOR /DNEED_JPEG_DECODER /DLIBDJVU_STATIC /D_CRT_SECURE_NO_DEPRECATE /D_CRT_NONSTDC_NO_DEPRECATE /D_SECURE_SCL=0 /I"src" /I"src\libdjvu" /Fo"%TEST_BASENAME%-scheduler.obj" /c "src\RenderScheduler.cpp"
if errorlevel 1 exit /b %errorlevel%
link /nologo /out:"%TEST_BASENAME%.exe" "%TEST_BASENAME%.obj" "%TEST_BASENAME%-scheduler.obj" /entry:wmainCRTStartup /subsystem:console /manifest:no
if errorlevel 1 exit /b %errorlevel%
"%TEST_BASENAME%.exe"
exit /b %errorlevel%
