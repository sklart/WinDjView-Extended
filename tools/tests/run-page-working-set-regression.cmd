...
set "TEST_BASENAME=tools\tests\page_working_set_regression"
...
cl %TEST_FLAGS% /Fo"%TEST_BASENAME%.obj" /c "tools\tests\page_working_set_regression.cpp"
if errorlevel 1 exit /b %errorlevel%
cl %TEST_FLAGS% /Fo"%TEST_BASENAME%-working-set.obj" /c "src\PageWorkingSet.cpp"
if errorlevel 1 exit /b %errorlevel%
link /nologo /out:"%TEST_BASENAME%.exe" "%TEST_BASENAME%.obj" "%TEST_BASENAME%-working-set.obj" /entry:wmainCRTStartup /subsystem:console /manifest:no
if errorlevel 1 exit /b %errorlevel%
"%TEST_BASENAME%.exe"
exit /b %errorlevel%
