	...
	: CPropertyPage(CSettingsDisplayPage::IDD), m_bSlidersInitialized(false)
{
	...
	m_bChangePagesColor = m_displaySettings.bChangePagesColor;
	m_bCropPages = m_displaySettings.bCropPages;
...
void CSettingsDisplayPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHANGE_PAGES_COLOR, m_bChangePagesColor);
	DDX_Check(pDX, IDC_CROP_PAGES, m_bCropPages);
...
BOOL CSettingsDisplayPage::OnInitDialog()
{
	...
	for (int nUnits = CAppSettings::Centimeters; nUnits <= CAppSettings::Pixels; ++nUnits)
	{
		CString strUnits;
...
BOOL CSettingsDisplayPage::OnKillActive()
{
	...
	m_displaySettings.bChangePagesColor = !!m_bChangePagesColor;
	m_displaySettings.bCropPages = !!m_bCropPages;
