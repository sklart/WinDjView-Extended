	...
	: CMyDialog(CDocPropertiesDlg::IDD, pParent), m_nSortBy(-1), m_bSortAsc(true)
{
	...
	m_strDocFullName = pSource->GetFilePath();
	m_strDocPath = m_strDocFullName.Left(m_strDocFullName.ReverseFind(_T('\\')) + 1);
	...
	if (pDir != NULL)
	{
		...
		for (int i = 0; i < m_nFiles; ++i)
		{
			...
			m_files[i].strPageTitle = MakeCString(pFile->get_title());
		}
	}
	else if (pDir0 != NULL)
	{
...
void CDocPropertiesDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_STATIC_PATH, m_strDocPath);
...
BEGIN_MESSAGE_MAP(CDocPropertiesDlg, CMyDialog)
	...
	ON_STN_CLICKED(IDC_STATIC_PATH, OnStnClickedStaticPath)
END_MESSAGE_MAP()
...
void CDocPropertiesDlg::PopulateFiles()
{
	...
	if (m_bShowAllFiles)
	{
		...
		for (int i = 0; i < (int)m_files.size(); ++i)
		{
			...
			m_listFiles.SetItemData(nIndex, (DWORD_PTR) &m_files[i]);
		}
...
void CDocPropertiesDlg::OnStnClickedStaticPath()
{
		AfxMessageBox(_T(" ") + m_strDocFullName, MB_OK | MB_ICONINFORMATION);
}
