...
CMDIChild::CMDIChild()
	: m_pContentWnd(NULL), m_pDocument(NULL), m_pThumbnailsView(NULL),
	  m_pContentsTree(NULL), m_pResultsView(NULL), m_pBMSearchTree(NULL), m_pBookmarksTree(NULL),
...
CBookmarksView* CMDIChild::GetBMSearchTree(bool bActivate)
{
	if (m_pBMSearchTree == NULL)
	{
		DjVuSource* pSource = m_pDocument->GetSource();
		m_pBMSearchTree = new CBookmarksView(pSource);
		m_pBMSearchTree->Create(NULL, NULL, WS_CHILD | WS_TABSTOP | TVS_HASLINES
			| TVS_LINESATROOT | TVS_HASBUTTONS | TVS_DISABLEDRAGDROP
			| TVS_SHOWSELALWAYS | TVS_TRACKSELECT, CRect(0, 0, 0, 0), &m_navPane, 6);
		m_navPane.AddTab(LoadString(IDS_SEARCH_CONTENTS_TAB), m_pBMSearchTree);
		m_navPane.SetTabSettings(m_pBMSearchTree, true);
		m_pBMSearchTree->OnInitialUpdate();
	}

	if (bActivate)
	{
		m_navPane.ActivateTab(m_pBMSearchTree);
		m_navPane.UpdateWindow();
	}

	return m_pBMSearchTree;
}
...
void CMDIChild::OnUpdate(const Observable* source, const Message* message)
{
	if (message->code == APP_LANGUAGE_CHANGED)
	{
		...
		if (m_pBMSearchTree != NULL)
			m_navPane.SetTabName(m_pBMSearchTree, LoadString(IDS_SEARCH_CONTENTS_TAB));
