...
public:
	...
	BOOL WriteProfileDocsSettings(LPCTSTR pszSection, const GUTF8String& value);
	...
	virtual bool LoadDocSettingsFromRegistry(const CString& strKey, DocSettings* pSettings);
	virtual bool LoadDocSettingsFromFiles(const CString& strKey, DocSettings* pSettings);
	virtual DictionaryInfo* GetDictionaryInfo(const CString& strPathName, bool bCheckPath = true);
	virtual void ReportFatalError();
	virtual bool GetCropPages() { return GetDisplaySettings()->bCropPages; }
	void SetCropLabel(bool bCrop = true ) { GetDisplaySettings()->bCropPages = bCrop; }
	void SetStatusMessageDefault();
	...
	BOOL CreateIniFile();
	BOOL FindIniFile(CString& lpszFileName);
	void OpenSession(CString& lpszFileName);
	void OpenLastSession();
	bool UpdateOpenLastSession();
	...
	CString GetCustomZoomListString(CString strDlgResult);
...
protected:
	...
	afx_msg void OnFileOpenSession();
	afx_msg void OnFileOpenLastSession();
	afx_msg void OnUpdateFileOpenLastSession(CCmdUI* pCmdUI);
