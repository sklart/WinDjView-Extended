...
CDocument* CMyDocManager::OpenDocumentFile(LPCTSTR lpszFileName)
{
	...
	if (strFileName.GetLength() - nPos == 8 && strFileName.Right(8) == _T(".session"))
	{
		theApp.OpenSession(strFileName);
		return NULL;
	}
	...
	if (pOpenDocument != NULL)
	{
		...
		if (!strPage.IsEmpty())
			pView->GoToURL(MakeUTF8String(_T("?djvuopts&pageno=") + strPage));

		if (!strQuery.IsEmpty())
			pView->GoToURL(MakeUTF8String(_T("?") + strQuery));
	}
