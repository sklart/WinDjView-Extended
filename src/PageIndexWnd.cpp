...
int CPageIndexWnd::AddEntries(const XMLNode& parent, HTREEITEM hParent)
{
	...
	for (it = parent.childElements.begin(); it != parent.childElements.end(); ++it)
	{
		...
		m_pList->SetItemData(hItem, m_entries.size() - 1, MakeCString(entry.strLink));
		entry.hItem = hItem;
...
void CPageIndexWnd::OnChangeText()
{
	...
	int left = 0, right = (int)m_entries/*m_sorted*/.size();
	while (left < right - 1)
	{
		int mid = (left + right) / 2;
		if (m_entries/*m_sorted*/[mid].strFirst <= text)
			left = mid;
		else
			right = mid;
	} 
	...
	while (left > 0 && m_entries/*m_sorted*/[left - 1].strFirst == text)
		--left;
	while (left > 0 && m_entries/*m_sorted*/[left - 1].strLast == text)
		--left;

	m_bChangeInternal = true;
	if (left >= 0 && left < static_cast<int>(m_entries/*m_sorted*/.size()))
		m_pList->SelectItem(m_entries/*m_sorted*/[left].hItem);
	else
