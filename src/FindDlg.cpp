	...
	: CMyDialog(CFindDlg::IDD, pParent)
{
	...
	m_bInContents = theApp.GetAppSettings()->bInContents;
	m_bIgnoreHyphens = theApp.GetAppSettings()->bIgnoreHyphens;
	m_bWholeWordsOnly = theApp.GetAppSettings()->bWholeWordsOnly;
...
void CFindDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_IN_CONTENTS, m_bInContents);
	DDX_Check(pDX, IDC_IGNORE_HYPHENS, m_bIgnoreHyphens);
	DDX_Check(pDX, IDC_WHOLE_WORDS_ONLY, m_bWholeWordsOnly);
}
...
void CFindDlg::OnFindPrev()
{
	...
	GetMainFrame()->SetMessageText(AFX_IDS_IDLEMESSAGE);
	GetMainFrame()->SendMessage(WM_COMMAND, ID_FIND_PREV);
...
void CFindDlg::UpdateButtons()
{
	...
	GetDlgItem(IDC_FIND_PREV)->EnableWindow(bEnable);
...
BOOL CFindDlg::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_F3)
	{
		...
		else if (!bControlPressed && bShiftPressed && !bAltPressed)
		{
			OnFindPrev();
			return true;
		}
