...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_UPDATE_COMMAND_UI(ID_MOUSE_POS, OnUpdateMousePos)
	ON_UPDATE_COMMAND_UI(ID_ROTATE_PAGES, OnUpdateRotatePages)
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	...
	ON_COMMAND(ID_VIEW_CROPPAGES, OnViewCropPages)
	ON_UPDATE_COMMAND_UI(ID_VIEW_CROPPAGES, OnUpdateViewCropPages)
END_MESSAGE_MAP()
...
void CMainFrame::InitToolBar()
{
	...
	m_wndToolBar.SetControl(nComboPage, IDC_PAGENUM, 110);
...
void CMainFrame::OnChangePageEdit()
{
	...
	m_cboPage.GetWindowText(strPageTitle);

	pView->GoToTitle(strPageTitle);
	pView->SetFocus();
}
...
void CMainFrame::OnDropDownPage()
{
	...
	if (pView->GetPageCount() > m_cboPage.GetCount())
	{
		...
		for (int i = m_cboPage.GetCount(); i < pView->GetPageCount(); ++i)
			m_cboPage.AddString(pView->GetPageTitle (i));
	}
...
void CMainFrame::UpdatePageCombo(const CDjVuView* pView)
{
	...
	int nPage = pView->GetCurrentPage();
	CString strPageTitle = pView->GetPageTitle(nPage);
	CString strCurPageTitle;
	m_cboPage.GetWindowText(strCurPageTitle);

	if (strPageTitle != strCurPageTitle)
		m_cboPage.SetWindowText(strPageTitle);
}
...
void CMainFrame::CloseAllMDIChilds()
{
	m_tabbedMDI.CloseAllTabs();
}
...
void CMainFrame::OnUpdateMousePos(CCmdUI* pCmdUI)
{
	...
	if (pView == NULL)
	{
		m_wndStatusBar.SetPaneInfo(3, ID_MOUSE_POS, SBPS_DISABLED | SBPS_NOBORDERS, 0);
		pCmdUI->Enable(false);
		return;
	}
	
	CString strNewMessage = pView->GetMousePosFromPage();

	CStatusBarCtrl& status = m_wndStatusBar.GetStatusBarCtrl();
	if (status.GetText(3, 0) != strNewMessage)
	{
		pCmdUI->SetText(strNewMessage);
		status.SetText(strNewMessage, 3, 0);

		CWindowDC dc(&status);
		CFont* pOldFont = dc.SelectObject(status.GetFont());
		m_wndStatusBar.SetPaneInfo(3, ID_MOUSE_POS, 0, dc.GetTextExtent(strNewMessage).cx + 2);
		dc.SelectObject(pOldFont);
	}
}
...
void CMainFrame::OnUpdateStatusSize(CCmdUI* pCmdUI)
{
	...
	if (pView == NULL)
	{
		m_wndStatusBar.SetPaneInfo(5, ID_INDICATOR_SIZE, SBPS_DISABLED | SBPS_NOBORDERS, 0);
		pCmdUI->Enable(false);
		return;
	}
	...
	double fWidth = nUnits == CAppSettings::Pixels ? szPage.cx : static_cast<int>(fUnitsPerInch * 100.0 * szPage.cx / nDPI + 0.5) * 0.01;
	double fHeight = nUnits == CAppSettings::Pixels ? szPage.cy : static_cast<int>(fUnitsPerInch * 100.0 * szPage.cy / nDPI + 0.5) * 0.01;
	...
	{
		pCmdUI->SetText(strNewMessage);
		status.SetText(strNewMessage, 5, 0);

		CWindowDC dc(&status);
		CFont* pOldFont = dc.SelectObject(status.GetFont());
		m_wndStatusBar.SetPaneInfo(5, ID_INDICATOR_SIZE, 0, dc.GetTextExtent(strNewMessage).cx + 2);
		dc.SelectObject(pOldFont);
	}
}

void CMainFrame::OnUpdateRotatePages(CCmdUI* pCmdUI)
{
	CDjVuView* pView = (CDjVuView*) GetActiveView();
	if (pView == NULL || pView->GetRotate() == 0)
	{
		m_wndStatusBar.SetPaneInfo(6, ID_ROTATE_PAGES, SBPS_DISABLED | SBPS_NOBORDERS, 0);
		pCmdUI->Enable(false);
		return;
	}
int nAngle = 0;
switch (pView->GetRotate())
		{
		case 1:
			nAngle = 270;
			break;
	...
	CString strNewMessage;
	strNewMessage.Format(ID_ROTATE_PAGES, nAngle);

	CStatusBarCtrl& status = m_wndStatusBar.GetStatusBarCtrl();
	if (status.GetText(6, 0) != strNewMessage)
	{
		pCmdUI->SetText(strNewMessage);
		status.SetText(strNewMessage, 6, 0);

		CWindowDC dc(&status);
		CFont* pOldFont = dc.SelectObject(status.GetFont());
		m_wndStatusBar.SetPaneInfo(6, ID_ROTATE_PAGES, 0, dc.GetTextExtent(strNewMessage).cx + 2);
		dc.SelectObject(pOldFont);
	}
}
...
void CMainFrame::LanguageChanged()
{
	...
	SetMessageTxt(AFX_IDS_IDLEMESSAGE);
}

void CMainFrame::OnClose()
{
	CString strINI;
	if (theApp.GetAppSettings()->bProgramSettingsIntoRegistry && theApp.FindIniFile(strINI))
	{		
		int nResult = AfxMessageBox(IDS_DELETE_INI_FILE, MB_ICONEXCLAMATION | MB_YESNOCANCEL);
		if (nResult == IDCANCEL)
			return;
	...
	if (pFindDlg != NULL)
	{
		...
		theApp.GetAppSettings()->bInContents = !!pFindDlg->m_bInContents;
		theApp.GetAppSettings()->bIgnoreHyphens = !!pFindDlg->m_bIgnoreHyphens;
		theApp.GetAppSettings()->bWholeWordsOnly = !!pFindDlg->m_bWholeWordsOnly;
	}
...
void CMainFrame::OnUpdate(const Observable* source, const Message* message)
{
	...
	else if (message->code == TAB_ACTIVATED)
	{
		...
		if (pWnd != NULL)
		{
			...
			for (int i = m_cboPage.GetCount() - 1; i >= 0; --i)
				m_cboPage.DeleteString(i);
...
void CMainFrame::OnMouseWheelPage(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	int nPage = pView->GetCurrentPage();
	nPage += (pNMWheel->bUp ? -1 : 1);
	if ((pView->GetLayout() == pView->Facing || pView->GetLayout() == pView->ContinuousFacing))
		if (pNMWheel->bUp && nPage > 0)
			--nPage;
		else if (!pNMWheel->bUp && nPage < pView->GetPageCount() - 1)
			++nPage;
...
void CMainFrame::OnUpdateViewCropPages(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(theApp.GetDisplaySettings()->bCropPages);
	pCmdUI->Enable(true);
}

void CMainFrame::OnViewCropPages()
{
	CDisplaySettings* pDisplaySettings = theApp.GetDisplaySettings();
	pDisplaySettings->bCropPages = !pDisplaySettings->bCropPages;
	theApp.UpdateObservers(APP_SETTINGS_CHANGED);
}

void CMainFrame::SetMessageTxt(UINT nID)
{
	CString strMsg;
	strMsg.Format(nID);
	if (strMsg != m_strOldMessageTxt)
	{
		m_strOldMessageTxt = strMsg;
		SetMessageText(nID);
	}
}

void CMainFrame::SetMessageTxt(LPCTSTR lpszText)
{
	if (lpszText != m_strOldMessageTxt)
	{
		m_strOldMessageTxt = lpszText;
