...
const TCHAR* s_pszChangePagesColor = _T("change-pages-color");
const TCHAR* s_pszCropPages = _T("crop-pages");
const TCHAR* s_pszPagesColor = _T("pages-color");
const TCHAR* s_pszPagesColorTransparency = _T("pages-color-transparency");
...
const TCHAR* s_pszNoRegistry = _T("no-registry");
...
const TCHAR* s_pszProgramSettingsIntoRegistry = _T("program-settings-into-registry");
const TCHAR* s_pszDocsSettingsIntoRegistry = _T("docs-settings-into-registry");
const TCHAR* s_pszRestoreView = _T("restore-view");
const TCHAR* s_pszOpenLastDocuments = _T("open-last-documents");
const TCHAR* s_pszFindBookmarkTitle = _T("find-bookmark-title");
const TCHAR* s_pszDisablePositioning = _T("disable-positioning");
const TCHAR* s_pszCustomZoomLevels = _T("custom-zoom-levels");
const TCHAR* s_pszCustomZoomList = _T("custom-zoom-list");
const TCHAR* s_pszDjVuEditor = _T("djvu-editor");
const TCHAR* s_pszVersion = _T("version");
const TCHAR* s_pszLanguage = _T("language");
const TCHAR* s_pszMatchCase = _T("match-case");
const TCHAR* s_pszIgnoreHyphens = _T("ignore-hyphens");
...
const TCHAR* s_pszLastOpenedDocumentsSection = _T("Last Opened Documents");
const TCHAR* s_pszLastOpenedDocumentPrefix = _T("file");
...
BEGIN_MESSAGE_MAP(CDjViewApp, CWinApp)
	...
	ON_COMMAND(ID_FILE_OPEN_SESSION, OnFileOpenSession)
	ON_COMMAND(ID_FILE_OPEN_LAST_SESSION, OnFileOpenLastSession)
	ON_UPDATE_COMMAND_UI(ID_FILE_OPEN_LAST_SESSION, OnUpdateFileOpenLastSession)
...
BOOL CDjViewApp::InitInstance()
{
	...
	CString strINI;
	if (FindIniFile(strINI))
	{	
		BOOL fMemTrack = AfxEnableMemoryTracking(FALSE);
		free(const_cast<LPTSTR>(m_pszProfileName));
		m_pszProfileName = _tcsdup(strINI);
		m_appSettings.bDocsSettingsIntoRegistry = false;
		m_appSettings.bProgramSettingsIntoRegistry = false;
	...
	if (m_appSettings.bOpenLastDocuments && __argc < 2)
	{
		// up to 100 files
		for (int nItem = 0; nItem < 100; nItem++)
		{
			CString strItem = GetProfileString(
				s_pszLastOpenedDocumentsSection,
				s_pszLastOpenedDocumentPrefix + FormatString(_T("%d"), nItem),
				_T("")
				);
			if (!strItem.IsEmpty())
			{
				m_pDocManager->OpenDocumentFile(strItem);
			}
			else
			{
				break;
...
CMainFrame* CDjViewApp::CreateMainFrame(bool bAppStartup, int nCmdShow)
{
	...
	if (bAppStartup)
	{
		...
		int nPos = cmdInfo.m_strFileName.ReverseFind('.');
		if (cmdInfo.m_strFileName.GetLength() - nPos == 8 && cmdInfo.m_strFileName.Right(8) == _T(".session"))
		{
			theApp.OpenSession(cmdInfo.m_strFileName);
		}
...
void CDjViewApp::LoadSettings()
{
	...
	m_appSettings.bNoRegistry = !!GetProfileInt(s_pszGlobalSection, s_pszNoRegistry, m_appSettings.bNoRegistry);
...
//	m_appSettings.bProgramSettingsIntoRegistry = !!GetProfileInt(s_pszGlobalSection, s_pszProgramSettingsIntoRegistry, m_appSettings.bProgramSettingsIntoRegistry);
	if (m_appSettings.bProgramSettingsIntoRegistry)
		m_appSettings.bDocsSettingsIntoRegistry = !!GetProfileInt(s_pszGlobalSection, s_pszDocsSettingsIntoRegistry, m_appSettings.bDocsSettingsIntoRegistry);
	m_appSettings.bRestoreView = !!GetProfileInt(s_pszGlobalSection, s_pszRestoreView, m_appSettings.bRestoreView);
	m_appSettings.bOpenLastDocuments = !!GetProfileInt(s_pszGlobalSection, s_pszOpenLastDocuments, m_appSettings.bOpenLastDocuments);
	m_appSettings.bFindBookmarkTitle = !!GetProfileInt(s_pszGlobalSection, s_pszFindBookmarkTitle, m_appSettings.bFindBookmarkTitle);
	m_appSettings.bDisablePositioning = !!GetProfileInt(s_pszGlobalSection, s_pszDisablePositioning, m_appSettings.bDisablePositioning);
	m_appSettings.bCustomZoomLevels = !!GetProfileInt(s_pszGlobalSection, s_pszCustomZoomLevels, m_appSettings.bCustomZoomLevels);
	m_appSettings.strCustomZoomList = GetProfileString(s_pszGlobalSection, s_pszCustomZoomList,  m_appSettings.strCustomZoomList);
	m_appSettings.strCustomZoomList = GetProfileString(s_pszGlobalSection, s_pszCustomZoomList,  m_appSettings.strCustomZoomList);
	m_appSettings.strCustomZoomList = GetCustomZoomListString(m_appSettings.strCustomZoomList);
	m_appSettings.strDjVuEditor = GetProfileString(s_pszGlobalSection, s_pszDjVuEditor,  m_appSettings.strDjVuEditor);
	...
	m_appSettings.bIgnoreHyphens = !!GetProfileInt(s_pszGlobalSection, s_pszIgnoreHyphens, m_appSettings.bIgnoreHyphens);
...
//	m_appSettings.bCheckUpdates = !!GetProfileInt(s_pszGlobalSection, s_pszCheckUpdates, m_appSettings.bCheckUpdates);
	...
	m_displaySettings.bChangePagesColor = !!GetProfileInt(s_pszDisplaySection, s_pszChangePagesColor, m_displaySettings.bChangePagesColor);
	m_displaySettings.bCropPages = !!GetProfileInt(s_pszDisplaySection, s_pszCropPages, m_displaySettings.bCropPages);
	m_displaySettings.nPagesColor = GetProfileInt(s_pszDisplaySection, s_pszPagesColor, m_displaySettings.nPagesColor);
	m_displaySettings.fPagesColorTransparency = GetProfileDouble(s_pszDisplaySection, s_pszPagesColorTransparency, m_displaySettings.fPagesColorTransparency);
	...
	if (m_appSettings.nUnits < CAppSettings::Centimeters || m_appSettings.nUnits > CAppSettings::Pixels)
		m_appSettings.nUnits = CAppSettings::Pixels;
...
bool CDjViewApp::LoadDocSettings(const CString& strKey, DocSettings* pSettings)
{
	if (m_appSettings.bDocsSettingsIntoRegistry)
		return LoadDocSettingsFromRegistry(strKey, pSettings);
	else
		return LoadDocSettingsFromFiles(strKey, pSettings);
}

bool CDjViewApp::LoadDocSettingsFromFiles(const CString& strKey, DocSettings* pSettings)
{
	...
	CString strFileMask = strPathName;
	if (!PathAppend(strFileMask.GetBuffer(MAX_PATH), _T("*")))
		return false;
	strFileMask.ReleaseBuffer();

	wstring strDjVuFileName, strLastKnownLocation;
	MakeWString(pSettings->strLastKnownLocation, strDjVuFileName);
	...
	if (hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			if (parser.Parse(in) && parser.GetRoot()->GetAttribute(s_pszLastKnownLocation, strLastKnownLocation))
			{
				if (strDjVuFileName == strLastKnownLocation)
				{
					pSettings->Load(*parser.GetRoot());
					bResult = true;
...
bool CDjViewApp::LoadDocSettingsFromRegistry(const CString& strKey, DocSettings* pSettings)
{
...
void CDjViewApp::SaveSettings()
{
	...
	WriteProfileInt(s_pszGlobalSection, s_pszOpenLastDocuments, m_appSettings.bOpenLastDocuments);
	WriteProfileInt(s_pszGlobalSection, s_pszFindBookmarkTitle, m_appSettings.bFindBookmarkTitle);
	WriteProfileInt(s_pszGlobalSection, s_pszDisablePositioning, m_appSettings.bDisablePositioning);
	WriteProfileInt(s_pszGlobalSection, s_pszCustomZoomLevels, m_appSettings.bCustomZoomLevels);
	WriteProfileString(s_pszGlobalSection, s_pszCustomZoomList, m_appSettings.strCustomZoomList);
	WriteProfileString(s_pszGlobalSection, s_pszDjVuEditor, m_appSettings.strDjVuEditor);
	...
	WriteProfileInt(s_pszGlobalSection, s_pszIgnoreHyphens, m_appSettings.bIgnoreHyphens);
...
//	WriteProfileInt(s_pszGlobalSection, s_pszCheckUpdates, m_appSettings.bCheckUpdates);
	...
	WriteProfileInt(s_pszDisplaySection, s_pszChangePagesColor, m_displaySettings.bChangePagesColor);
	WriteProfileInt(s_pszDisplaySection, s_pszCropPages, m_displaySettings.bCropPages);
	WriteProfileInt(s_pszDisplaySection, s_pszPagesColor, m_displaySettings.nPagesColor);
	WriteProfileDouble(s_pszDisplaySection, s_pszPagesColorTransparency, m_displaySettings.fPagesColorTransparency);
	...
	for (map<MD5, DocSettings>::const_iterator it = settings.begin(); it != settings.end(); ++it)
	{
		...
		if (m_appSettings.bDocsSettingsIntoRegistry || !WriteProfileDocsSettings(strKey, xml))
		{
			if (m_appSettings.bNoRegistry)
				break;
			if (!m_appSettings.bProgramSettingsIntoRegistry)
			{
				SetRegistryKey(_T("Andrew Zhezherun"));
				m_appSettings.bProgramSettingsIntoRegistry = true;
				m_appSettings.bDocsSettingsIntoRegistry = true;
			}
...
//	WriteProfileInt(s_pszGlobalSection, s_pszProgramSettingsIntoRegistry, m_appSettings.bProgramSettingsIntoRegistry);
	WriteProfileInt(s_pszGlobalSection, s_pszDocsSettingsIntoRegistry, m_appSettings.bDocsSettingsIntoRegistry);
...
BOOL CDjViewApp::WriteProfileDocsSettings(LPCTSTR pszSection, const GUTF8String& value)
{
	CString strPathName, strFileName, strTemp;
	GetModuleFileName(NULL, strTemp.GetBuffer(_MAX_PATH), _MAX_PATH);
	strTemp.ReleaseBuffer(strTemp.ReverseFind('\\') + 1);
	...
	if (!PathIsDirectory(strPathName) && !CreateDirectory(strPathName, NULL))
	{
		if (m_appSettings.bNoRegistry)
		{
			...
			AfxMessageBox(IDS_CANNOT_CREATE_INI, MB_OK | MB_ICONEXCLAMATION);
		return false;
	}
	...
	file.Write(out.str().c_str(), value.length());
	file.Close();
...
void CDjViewApp::OnFileOpenSession()
{
	CMyFileDialog dlg(true, _T("session"), NULL, OFN_HIDEREADONLY | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST,
		LoadString(IDS_SESSION_FILTER));

	CString strTitle;
	strTitle.LoadString(IDS_OPEN_SESSION);
	dlg.m_ofn.lpstrTitle = strTitle.GetBuffer(0);

	//if (dlg.DoModal() != IDOK)
	//	return;
	if (dlg.DoModal() == IDOK)
		OpenSession(dlg.GetPathName());
	/*CWaitCursor wait;
	...
	if (!parser.Parse(in))
	{
		AfxMessageBox(IDS_CANNOT_READ_SESSION, MB_OK | MB_ICONEXCLAMATION);
		return;
	}
	...
	for (it = node->childElements.begin(); it != node->childElements.end(); ++it)
	{
		...
		if (child.tagName == _T("file"))
		{
			map<wstring, wstring>::const_iterator it2 = child.attributes.begin();
			if (it2->first == _T("path"))
			{
				CString path = MakeCString(it2->second);
...
void CDjViewApp::OpenLastSession()
{
	OnFileOpenLastSession();
}

bool CDjViewApp::UpdateOpenLastSession()
{
	bool bResult = false;
	for (int nItem = 0; nItem < 100; nItem++)
	{
...
void CDjViewApp::OnFileOpenLastSession()
{
	...
	for (list<CMainFrame*>::iterator it = m_frames.begin(); it != m_frames.end(); ++it)
	{
		...
		{
			pDoc = (CDjVuDoc*)pMainFrame->GetActiveDocument();
			break;
		}
	}
	for (int nItem = 0; nItem < 100; nItem++)
...
{
	pCmdUI->Enable(UpdateOpenLastSession());
}
...
void CDjViewApp::OnFileSettings()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		m_appSettings.bOpenLastDocuments = !!dlg.m_pageAdvanced.m_bOpenLastDocuments;
		m_appSettings.bFindBookmarkTitle = !!dlg.m_pageAdvanced.m_bFindBookmarkTitle;
		m_appSettings.bDisablePositioning = !!dlg.m_pageAdvanced.m_bDisablePositioning;
		m_appSettings.bCustomZoomLevels = !!dlg.m_pageAdvanced.m_bCustomZoomLevels;
		m_appSettings.strCustomZoomList = GetCustomZoomListString(dlg.m_pageAdvanced.m_strCustomZoomList);
//		m_appSettings.bCheckUpdates = !!dlg.m_pageAdvanced.m_bCheckUpdates;
		...
		if ((m_appSettings.bDocsSettingsIntoRegistry == !dlg.m_pageGeneral.m_bDocsSettingsIntoRegistry) &&
			dlg.m_pageGeneral.m_bDocsSettingsIntoRegistry == 0)
		{
			if (m_pszRegistryKey != NULL)
			{
				vector<CString> keys;
				if (EnumProfileKeys(s_pszDocumentsSection, keys) && keys.size() > 0 &&
					AfxMessageBox(IDS_FROM_REG_TO_DIR, MB_ICONQUESTION | MB_YESNO) == IDYES)
				{
					for (size_t i = 0; i < keys.size(); ++i)
					{
						DocSettings tempSettings;
						CString strPrevKey = s_pszDocumentsSection + CString(_T("\\")) + keys[i];
						CString strFileName = GetProfileString(strPrevKey, s_pszLastKnownLocation, _T(""));
						GUTF8String strSettings;
						if (GetProfileCompressed(strPrevKey, s_pszSettings, strSettings))
						{
							...
							if (parser.Parse(sin))
							{
								tempSettings.Load(*parser.GetRoot());
								if (tempSettings.strLastKnownLocation == "" && strFileName != _T(""))
									tempSettings.strLastKnownLocation = strFileName;
								GUTF8String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + tempSettings.GetXML();
								if (!WriteProfileDocsSettings(keys[i], xml))
								{
									dlg.m_pageGeneral.m_bDocsSettingsIntoRegistry = 1;
									dlg.m_pageGeneral.m_bProgramSettingsIntoRegistry = 1;
									break;
								}
							}
						}
		...
		m_appSettings.bDocsSettingsIntoRegistry = !!dlg.m_pageGeneral.m_bDocsSettingsIntoRegistry;

		if ((m_appSettings.bProgramSettingsIntoRegistry == !dlg.m_pageGeneral.m_bProgramSettingsIntoRegistry) &&
			dlg.m_pageGeneral.m_bProgramSettingsIntoRegistry == 0)
		{
			if (CreateIniFile())
			{
			...
			if (dlg.m_pageGeneral.m_bProgramSettingsIntoRegistry == 1 && m_pszRegistryKey == NULL)
				SetRegistryKey(_T("Andrew Zhezherun"));

		m_appSettings.bProgramSettingsIntoRegistry = !!dlg.m_pageGeneral.m_bProgramSettingsIntoRegistry;
...
BOOL CDjViewApp::FindIniFile(CString& strFileName)
{
	CString strTemp;
	GetModuleFileName(NULL, strTemp.GetBuffer(_MAX_PATH), _MAX_PATH);
	strTemp.ReleaseBuffer(strTemp.ReverseFind('.'));
	strFileName =  strTemp + _T(".ini");
...
BOOL CDjViewApp::CreateIniFile()
{
	if (!PathFileExists(m_pszProfileName))
	{
		...
		BOOL fMemTrack = AfxEnableMemoryTracking(FALSE);
		free(const_cast<LPTSTR>(m_pszProfileName));
		GetModuleFileName(m_hInstance, strTemp.GetBuffer(_MAX_PATH), _MAX_PATH);
		strTemp.ReleaseBuffer();
		m_pszProfileName = _tcsdup(strTemp);
		AfxEnableMemoryTracking(fMemTrack);
		PathRenameExtension(const_cast<LPTSTR>(m_pszProfileName), _T(".ini"));
	}
	else
		return true;

	CFile file;
	static const unsigned char szBuffer[270] =
	{
		
	};

	if (file.Open(m_pszProfileName, CFile::modeCreate | CFile::modeReadWrite))
	{
		file.Write(szBuffer, sizeof(szBuffer));
		file.Close();
		return true;
	}
	else if (m_appSettings.bNoRegistry)
	{
		::MessageBeep(MB_OK);
		UpdateObservers(SETTINGS_NOT_SAVED);
		return false;
...
BOOL CDjViewApp::SaveAllModified()
{
	...
	for (int nItem = 0; nItem < 100; nItem++)
	{
		...
		{
			// error deleteting - files ended
			break;
		}
	}

	// add current
	if (m_appSettings.bOpenLastDocuments)
	{
		int nItem = 0;
...
void CDjViewApp::UpdateFindDlg(CWnd* pNewParent)
{
	...
	m_appSettings.bInContents = !!m_pFindDlg->m_bInContents;
	m_appSettings.bIgnoreHyphens = !!m_pFindDlg->m_bIgnoreHyphens;
	m_appSettings.bWholeWordsOnly = !!m_pFindDlg->m_bWholeWordsOnly;
...
CString CDjViewApp::GetCustomZoomListString(CString strDlgResult)
{
	CString strResult = _T("");
	CString strLocale = _tsetlocale(LC_NUMERIC, NULL);
	_tsetlocale(LC_NUMERIC, _T("C"));
	strDlgResult.Replace(';', ',');
	m_appSettings.customZoomList.clear();
	if (strDlgResult.GetLength() > 3)
	{
		for (int nPos = 0; nPos >= 0; )
		{
			int nPrevPos = nPos;
			nPos = strDlgResult.Find(',', nPos);

			CString strValue;
			if (nPos == -1)
				strValue = strDlgResult.Right(strDlgResult.GetLength() - nPrevPos);
			else
			{
				strValue = strDlgResult.Mid(nPrevPos, nPos - nPrevPos);
				++nPos;
			}
			...
			{
				fValue = max(10, min(800, fValue));
				m_appSettings.customZoomList.push_back(fValue);
			}
		}
	}

	m_appSettings.customZoomList.sort();
	list<double>::iterator it;
	for (it = m_appSettings.customZoomList.begin(); it != m_appSettings.customZoomList.end();)
	{
		list<double>::iterator itPrev = it;
		if (++it != m_appSettings.customZoomList.end() && *itPrev == *it)
		{
			m_appSettings.customZoomList.erase(it);
			it = itPrev;
		}
	}
	if (m_appSettings.customZoomList.size() < 2)
		m_appSettings.customZoomList.clear();

	if (m_appSettings.customZoomList.size() > 0)
	{
		for (it = m_appSettings.customZoomList.begin(); ;)
		{
			strResult += FormatDouble(*it);
			if (++it == m_appSettings.customZoomList.end())
				break;
			strResult += _T(", ");
		}
	}
...
void CDjViewApp::SetStatusMessageDefault()
{
	CMainFrame* pMainFrame = (CMainFrame*) m_pMainWnd;
	pMainFrame->m_strOldMessageTxt = _T(""); 
}
