...
void CThumbnailsView::DrawPage(CDC* pDC, int nPage)
{
	...
	COLORREF color = (nPage == m_nCurrentPage ? /*clrBtnface*/0xFFFF : clrWindow);
...
void CThumbnailsView::OnInitialUpdate()
{
	...
	m_displaySettings.bChangePagesColor = false;
	m_displaySettings.bCropPages = false;
...
LRESULT CThumbnailsView::OnShowSettings(WPARAM wParam, LPARAM lParam)
{
	...
	if (nID == ID_THUMBNAILS_REDUCE)
		...
		ExportSelectedPages();
	else if (nID == ID_EXPORT_ALL_PAGES)
		ExportAllPages();
...
void CThumbnailsView::SettingsChanged()
{
	...
	appSettings.bChangePagesColor = false;
	appSettings.bCropPages = false;
...
void CThumbnailsView::ExportAllPages()
{
	set<int> selection;
	for (int nPage = 0; nPage < m_nPageCount; ++nPage)
		selection.insert(nPage);
