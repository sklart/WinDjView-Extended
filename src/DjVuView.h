...
bool HitResult(const GPList<DjVmNav::DjVuBookMark>& bookmarks,GUTF8String& text, BOOL& MatchCase);
...
public:
	...
	bool GoToTitle(CString strPageTitle, bool bAddHistoryPoint = true);
	...
	void GoToSelection(int nPage, int nStartPos, int nEndPos, bool bIsFindResult = false);
	...
	GUTF8String ChangeYoYe(const GUTF8String& text, bool bReplaceNBS = false);
	GUTF8String TrimNonChar(const GUTF8String& text, bool bTrimStart = true);
	GUTF8String DelNonCharOrDigit(const GUTF8String& text);
	GUTF8String TrimEnding(const GUTF8String& text, bool bOnlySpace = false);
	...
	void SetStrForSearch(int nPage, int nStartPos, int nEndPos);
	void SetStrForSearch(const GUTF8String& text);
	GUTF8String ModStrForSearch(const GUTF8String& text);
...
protected:
	...
	void DeleteBitmaps(bool bReloadSize = false);
	...
	bool ParseCGI(GUTF8String& cgiLink, int& Num, int& X, int& Y);
	bool ParseCGI(GUTF8String& cgiLink, int& Num, int& X, int& Y, bool& bIsText, bool& bIsRectCoord);
	bool FindBookmarkTitle(GUTF8String strBookmarkTitle, int nPage, Bookmark& bkm, int nSelPos = -1, bool bDelEndings = false);
	bool HasBookmarkPositioning(const Bookmark& bookmark);
	...
	bool HasCustomAnnoInDoc();
	bool HasCustomAnnoInPage(int nPage);
	GUTF8String TextUnderAnno(Page& page, Annotation* anno);
	bool IsRectCoord(const GUTF8String& strText);
	bool IsRectCoord(const GUTF8String& strText, list<GRect>& rects, bool bReturnCoord = false);
	void SelectRectangles(int nPage, const GUTF8String& strText);
	void SelectRectRange(int nPage, list<GRect>& rects,	bool& bInfoLoaded, CWaitCursor*& pWaitCursor);
	...
	void SelectTextRange(int nPage, int nStart, int nEnd,
			bool& bInfoLoaded, CWaitCursor*& pWaitCursor, bool bIsFindResult = false);
	void AddPrevEmptyWord(DjVuSelection& sel, DjVuTXT* pText) const;
	bool m_bHasSelection;
	int m_nSelectionPage;
	GRect m_rcSelection;
	CRect CheckWhiteMargins(int nPage);
	...
	struct HistoryPoint
	{
		bool operator==(const HistoryPoint& rhs) const
		{
			...
			int nOffsetY = abs(bmView.ptOffset.y - rhs.bmView.ptOffset.y);
			int nOffsetX = abs(bmView.ptOffset.x - rhs.bmView.ptOffset.x);
			//return bmView.nPage == rhs.bmView.nPage
			//		&& bmView.ptOffset == rhs.bmView.ptOffset;
			return bmView.nPage == rhs.bmView.nPage
					&& nOffsetY < 3 && nOffsetX < 3;
		}
		bool operator!=(const HistoryPoint& rhs) const
			{ return !(*this == rhs); }
...
protected:
	...
	afx_msg void OnUpdateRotateReset(CCmdUI *pCmdUI);
	...
	afx_msg void OnRedLine(UINT nID);
	...
	afx_msg void OnUpdateSelectPage(CCmdUI* pCmdUI);
	...
	afx_msg void OnDeleteAnnotationPage();
	afx_msg void OnDeleteAnnotationAll();
	afx_msg void OnSetPagesColor();
	afx_msg void OnUpdateDeleteAnnotationPage(CCmdUI* pCmdUI);
	afx_msg void OnUpdateDeleteAnnotationAll(CCmdUI* pCmdUI);
	afx_msg void OnAddBookmark();
	afx_msg void OnDeleteAllBookmarks();
	afx_msg void OnUpdateDeleteAllBookmarks(CCmdUI* pCmdUI);
	...
	afx_msg void OnCropPages();
	afx_msg void OnUpdateCropPages(CCmdUI *pCmdUI);
	DECLARE_MESSAGE_MAP()
};
