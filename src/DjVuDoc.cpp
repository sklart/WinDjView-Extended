...
BEGIN_MESSAGE_MAP(CDjVuDoc, CDocument)
	...
	ON_COMMAND(ID_EXPORT_ALL_PAGES, OnExportAllPages)
	...
	ON_COMMAND(ID_DOC_METADATA, OnFileDocMetadata)
	ON_UPDATE_COMMAND_UI(ID_DOC_METADATA, OnUpdateFileDocMetadata)
	...
	ON_COMMAND(ID_FILE_EDIT_DOC, OnFileEditDoc)
END_MESSAGE_MAP()
...
void CDjVuDoc::OnExportAllPages()
{
	set<int> selection;
	for (int nPage = 0; nPage < m_pSource->GetPageCount(); ++nPage)
		selection.insert(nPage);
	m_pSource->GetSettings()->UpdateObservers(PageRangeMsg(EXPORT_PAGES, selection));
}
...
void CDjVuDoc::OnFileDocMetadata()
{
	GUTF8String strInfo = m_pSource->GetDocMetaData();
	if (strInfo.length() > 0)
	{
		CDjViewApp::MessageBoxOptions mbo;
		mbo.bVerbatim = true;
		theApp.DoMessageBox(LoadString(IDS_DOC_METADATA) + MakeCString(strInfo), MB_ICONINFORMATION | MB_OK, 0, mbo);
	}
}

void CDjVuDoc::OnUpdateFileDocMetadata(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(m_pSource->GetDocMetaData().length() > 0);
}
...
void CDjVuDoc::OnFileOpenSession()
{
	...
	if (!bShiftPressed)
		GetDjVuView()->GetMainFrame()->CloseAllMDIChilds();
...
void CDjVuDoc::OnFileOpenLastSession()
{
	theApp.OpenLastSession();
}

void CDjVuDoc::OnUpdateFileOpenLastSession(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(theApp.UpdateOpenLastSession());
}

void CDjVuDoc::OnFileSaveSession()
{
	CString strPathName;
	CMyFileDialog dlg(false, _T("session"), strPathName, OFN_OVERWRITEPROMPT |
		OFN_HIDEREADONLY | OFN_NOREADONLYRETURN | OFN_PATHMUSTEXIST,
		LoadString(IDS_SESSION_FILTER));

	CString strTitle;
	strTitle.LoadString(IDS_SAVE_SESSION);
	dlg.m_ofn.lpstrTitle = strTitle.GetBuffer(0);
	...
	{
		CDocument *pDoc = *it;
		GUTF8String str = MakeUTF8String(pDoc->GetPathName());
		strXML += "<file path=\"" + str + "\" />\n";
	}
		strXML += "</session>";
	// Convert line endings.
	stringstream out;
...
void CDjVuDoc::OnFileDelete()
{
	if (AfxMessageBox(IDS_PROMPT_FILE_DELETE, MB_ICONEXCLAMATION | MB_OKCANCEL) == IDOK)
	{
...
void CDjVuDoc::OnFileReload()
{
	CString csFileName = m_pSource->GetFileName();
	OnFileClose();
	theApp.OpenDocumentFile(csFileName);
}
...
void CDjVuDoc::OnFileEditDoc()
{
	CString strEditor = theApp.GetAppSettings()->strDjVuEditor.MakeLower().Trim();
	int nParam = 4;
	int nSpace = strEditor.ReverseFind(_T(' '));
	if (nSpace > 0)
	{
		CString strEnd = strEditor.Right(strEditor.GetLength() - nSpace - 1);
		if (strEnd == _T("1"))
		{
			nParam = 1;
			strEditor = strEditor.Left(nSpace);
		}
	}
	CString strFileFullName = m_pSource->GetFilePath();
	CString strPath = strFileFullName.Left(strFileFullName.ReverseFind(_T('\\')) + 1);
	strPath.Replace(_T("\\"),_T("\\\\"));
	CString strFileName = strFileFullName.Right(strFileFullName.GetLength() - strFileFullName.ReverseFind(_T('\\')) - 1);
	int nDot = strFileName.ReverseFind(_T('.'));
	if (nDot > 0)
		strFileName = strFileName.Left(nDot);
	bool bShiftPressed = (::GetKeyState(VK_SHIFT) & 0x8000) != 0;
	CMyFileDialog dlg(true, _T(""), NULL, OFN_HIDEREADONLY | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST,
		LoadString(IDS_EDITORS_FILTER));

	CString strTitle;
	strTitle.LoadString(IDS_SELECT_EDITOR);
	dlg.m_ofn.lpstrTitle = strTitle.GetBuffer(0);

	if ((bShiftPressed || !PathFileExists(strEditor)))
	{
		if (dlg.DoModal() == IDOK)
		{
			strEditor = dlg.GetPathName().MakeLower();
			nParam = 4;
		}
		else 
			return;
	...
	if (PathFileExists(strEditor))
	{
		CString strParam;
		int nNMEditor = strEditor.Find(_T("djvunmeditor.exe"));
		CString strPage = MakeCString(GetDjVuView()->GetCurrentPage() + 1);
		if (nNMEditor >= 0 && nNMEditor == strEditor.GetLength() - 16)
		{
			strParam = _T("\"") + strFileFullName + _T("#") + strPage + _T("\"");
		}
		else
		{
			strParam = _T("\"") + strFileFullName + _T("\"");
			if (nParam > 1)
			{
				strParam += _T(" ") + strPage + _T(" \"") + strPath + _T("\" \"") + strFileName + _T("\"");
			}
		}

		::ShellExecute(NULL, _T("open"), strEditor, strParam, NULL, SW_SHOW);
		if (nParam == 1)
			strEditor += _T(' ') + MakeCString(nParam);
		theApp.GetAppSettings()->strDjVuEditor = strEditor;
	}
}
