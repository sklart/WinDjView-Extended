...
struct CDisplaySettings
{
	CDisplaySettings() :
			...
			bInvertColors(false), bChangePagesColor(false), bCropPages(false),
			nPagesColor(43733), fPagesColorTransparency(0.93){}
	...
	bool operator==(const CDisplaySettings& rhs) const
	{
		return GetBrightness() == rhs.GetBrightness() && GetContrast() == rhs.GetContrast()
				...
				&& bChangePagesColor == rhs.bChangePagesColor && nPagesColor == rhs.nPagesColor
				&& fPagesColorTransparency == rhs.fPagesColorTransparency && bCropPages == rhs.bCropPages;
	}
	...
	bool bScaleColorPnm;
	bool bScaleSubpix;
	bool bChangePagesColor;
	bool bCropPages;
	bool bInvertColors;
	int nPagesColor;
	double fPagesColorTransparency;
};
...
struct CAppSettings
{
	...
	bool bProgramSettingsIntoRegistry;
	bool bDocsSettingsIntoRegistry;
	bool bRestoreView;
	bool bOpenLastDocuments;
	bool bFindBookmarkTitle;
	bool bDisablePositioning;
	bool bCustomZoomLevels;
	CString strCustomZoomList;
	list<double> customZoomList;
	CString strDjVuEditor;
