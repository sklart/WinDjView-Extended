...
int CMyTreeView::PaintNode(CDC* pDC, TreeNode* pNode, const CRect& rcClip)
{
	...
	if (pNode != m_pRoot && rcIntersect.IntersectRect(rcClip, rcLine - ptScroll))
	{
		...
		else
		{
			...
			if (pNode->nLink == "")
			{
				crText = ::GetSysColor(pNode == m_pHoverNode ? COLOR_HOTLIGHT : COLOR_GRAYTEXT);
			}

			else 
				crText = ::GetSysColor(pNode == m_pHoverNode ? COLOR_HOTLIGHT : COLOR_WINDOWTEXT);
...
void CMyTreeView::SetHoverNode(TreeNode* pNode)
{
	...
	theApp.SetStatusMessageDefault();
	...
	if (pNode != NULL)
	{
		...
		if (pNode->nLink.IsEmpty())
			GetTopLevelFrame()->SetMessageText(IDS_HAS_NO_LINK);
		else
			GetTopLevelFrame()->SetMessageText(pNode->nLink);
...
void CMyTreeView::ExpandParent(HTREEITEM hItem)
{
	if (hItem == TVI_ROOT)
		return;
	...
	for (TreeNode* pParent = pNode->pParent; pParent != NULL; pParent = pParent->pParent)
		ExpandNode(pParent);
}
...
void CMyTreeView::SetItemData(HTREEITEM hItem, DWORD_PTR dwData, CString& nLink)
{
	TreeNode* pNode = (TreeNode*) hItem;
...
bool CMyTreeView::HasCollapsedChildren(TreeNode* pNode)
{
	bool result = false;
	for (; pNode != NULL; pNode = pNode->pNext)
	{
		if (pNode->pChild != NULL && pNode->bCollapsed)
			result = true;
		else 
			result = HasCollapsedChildren(pNode->pChild);
	...
	case WM_LBUTTONDBLCLK:
		{
			...
			m_pTree->OnLButtonDblClk(nFlags, point);
		}
		return true;
