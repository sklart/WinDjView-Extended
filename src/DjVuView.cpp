	...
	for (int nPage = 0; nPage < m_nPageCount; ++nPage)
	{
		...
		while (nHyphen != string::npos)
		{
			...
			for (nWordPos = nHyphen + 1;
				 ...
				 ++nWordPos)
			{
				if (wcschr(L"\n\r\013\035\037\038", text[nWordPos]) != NULL) //������� 037: const char DjVuTXT::end_of_paragraph = 037; // US: Unit Separator
					bFoundEOL = true;
			}
...
void CDjVuView::GetTextWithoutHyphen(GUTF8String& text)
{
	...
	while (nHyphen != string::npos)
	{
		...
		for (nWordPos = nHyphen + 1;
			...
			++nWordPos)
		{
			if (wcschr(L"\n\r\013\035\037\038", wtext[nWordPos]) != NULL) //������� 037: const char DjVuTXT::end_of_paragraph = 037; // US: Unit Separator
				bFoundEOL = true;
		}
...
GUTF8String CDjVuView::ChangeYoYe(const GUTF8String& text, bool bReplaceNBS)
{
	...
	{
		CString strYo = _T("�");
		CString nbSpace = CString((wchar_t)0xA0); //������ ������������ ������� �� �������
...
GUTF8String CDjVuView::TrimEnding(const GUTF8String& text, bool bOnlySpace)
{
	...
	else if (text.length() > 5)
	{
		...
		if (wcschr(L"�����", wtext[nLastPos]) != NULL)
		{
			strResult = MakeUTF8String(wtext.substr(0, nLastPos));
		}
		else if (wcschr(L"�", wtext[nLastPos]) != NULL && wcschr(L"���", wtext[nBeforeLast]) != NULL)
		{
			strResult = MakeUTF8String(wtext.substr(0, nBeforeLast));
		}
		else if (wcschr(L"�", wtext[nLastPos]) != NULL)
		{
			size_t n = wcschr(L"��", wtext[nBeforeLast]) != NULL ? nBeforeLast : nLastPos;
