...
public:
	...
	void SetItemData(HTREEITEM hItem, DWORD_PTR dwData, CString& nLink);
	DWORD_PTR GetItemData(HTREEITEM hItem);
	HTREEITEM GetSelectedItem();
	void SelectItem(HTREEITEM hItem);
	bool Expand(HTREEITEM hItem, UINT nCode);
	void ExpandParent(HTREEITEM hItem);
...
protected:
	...
	struct TreeNode
	{
		TreeNode(LPCTSTR pszItem, int nImageIndex, int nSelectedIndex, TreeNode* pParentNode)
			...
			pChild(NULL), pLastChild(NULL), pNext(NULL), bCollapsed(true), dwUserData(0), nLink(MakeCString("")) {}
		~TreeNode();
	...
	bool HasCollapsedChildren(TreeNode* pNode);
